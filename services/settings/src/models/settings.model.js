...
export class SettingsModel {
    ...
    static async getSettings(userId) {
        const result = await pgPool.query(`SELECT * FROM lp_settings WHERE user_id = $1`, [userId]);
        return result.rows[0] || null;
    }
    // Create settings
    static async createSettings(data) {
        try {
            const result = await pgPool.query(`INSERT INTO lp_settings (user_id,theme,sound_effect,speaking_service,reading_service,writing_service,listening_service,practice_service,quiz_service,notifications,language)
    ...
    static async createSettingsIfNotExists(user_id) {
        try {
            const result = await pgPool.query(`INSERT INTO lp_settings (user_id, language, theme) VALUES ($1, 'en', 'light') RETURNING * ON CONFLICT (user_id) DO NOTHING`, [user_id]);
            return result.rows[0];
        }
        catch (err) {
            // Handle unique user_id violation (duplicate row)
    ...
    static async updateSettings(userId, data) {
        const result = await pgPool.query(`UPDATE lp_settings
