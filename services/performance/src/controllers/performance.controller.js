...
export const updatePerformanceController = async (req, res, next) => {
    ...
    try {
        ...
        const performance = await PerformanceModel.updatePerformance(user_id, score);
        res.status(200).send({
...
export const getPerformanceController = async (req, res, next) => {
    try {
        const { userId } = req.params;
        if (!userId) {
